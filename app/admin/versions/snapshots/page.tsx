// app/admin/versions/snapshots/page.tsx
"use client";
...
import { ArrowLeft, Clock, Package, Box, AlertTriangle, RefreshCw } from "lucide-react";
...
export default function SnapshotsPage() {
  ...
  const fetchSnapshots = async () => {
    try {
      setLoading(true);
      const response = await fetch("/api/admin/versions?isSnapshot=true&limit=20");
  ...
  return (
    <div className="px-4 lg:px-6">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h2 className="text-2xl font-semibold text-white">Snapshot Versions</h2>
          <p className="text-sm text-white/60 mt-1">
            View all snapshot Minecraft versions
          </p>
        </div>
        <Button
          variant="outline"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {loading ? (
          ...
          <div className="col-span-3 text-center py-12 text-white/40">
            ...
            <p className="mt-3">No snapshot versions found</p>
          </div>
        ) : (
