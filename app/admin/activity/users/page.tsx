// app/admin/activity/users/page.tsx
"use client";
...
export default function UserActivitiesPage() {
  ...
  const fetchUserActivities = async () => {
    try {
      ...
      const response = await fetch(`/api/admin/activity/users?${params}`);
  ...
  const getActionColor = (action: string) => {
    ...
    if (action.includes("BANNED") || action.includes("REVOKE")) {
      return "bg-red-500/20 text-red-400 border-red-500/30";
    }
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-2xl font-semibold text-white">User Activities</h2>
      ...
      {userSummary.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          <div className="rounded-lg border border-white/10 bg-black/40 p-4">
            ...
            <div className="text-2xl font-bold text-white">{userSummary.length}</div>
          </div>
          <div className="rounded-lg border border-white/10 bg-black/40 p-4">
            <div className="text-sm text-white/40">Most Active User</div>
            <div className="text-lg font-bold text-white truncate">
              {userSummary[0]?.name || "N/A"}
            </div>
            <div className="text-xs text-white/40">
              {userSummary[0]?.activity_count || 0} activities
            </div>
          </div>
          <div className="rounded-lg border border-white/10 bg-black/40 p-4">
            <div className="text-sm text-white/40">Total Activities</div>
            <div className="text-2xl font-bold text-white">
              {userSummary.reduce((acc, user) => acc + (user.activity_count || 0), 0)}
            </div>
          </div>
          <div className="rounded-lg border border-white/10 bg-black/40 p-4">
            <div className="text-sm text-white/40">Admin Users</div>
            <div className="text-2xl font-bold text-purple-400">
              {userSummary.filter(u => u.role === "ADMIN").length}
            </div>
          </div>
        </div>
      )}
            ...
            {loading ? (
              ...
              activities.map((activity) => (
                <TableRow key={activity.id} className="border-white/10 hover:bg-white/5">
                  ...
                  <TableCell>
                    <Badge variant="outline" className={activity.user.role === "ADMIN" ? "border-purple-500/30 text-purple-400" : "border-white/10 text-white/60"}>
                      {activity.user.role}
                    </Badge>
                  </TableCell>
                  <TableCell>
